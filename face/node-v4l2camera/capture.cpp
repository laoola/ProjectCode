            ...
            if (frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
                ...
                for (uint32_t k = 0; ; k++) {
                    ...
                    if (frmival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
                        ...
                        ret->head =
                                (camera_format_t*)realloc(ret->head, (ret->length + 1) * sizeof (camera_format_t));
...
camera_controls_menus(const camera_t* camera, camera_control_t* control)
{
    ...
    control->menus.head = (camera_menu_t*)calloc(control->menus.length, sizeof (camera_menu_t));
...
camera_controls_query(const camera_t* camera, camera_control_t* control_list)
{
    ...
    for (uint32_t cid = V4L2_CID_USER_BASE; cid < V4L2_CID_LASTP1; cid++) {
        ...
        control->type = (camera_control_type_t)qctrl.type;
...
camera_controls_t* camera_controls_new(const camera_t* camera)
{
    ...
    camera_controls_t* controls = (camera_controls_t*)malloc(sizeof (camera_controls_t));
    controls->length = control_list_last - control_list;
    controls->head = (camera_control_t*)calloc(controls->length, sizeof (camera_control_t));
